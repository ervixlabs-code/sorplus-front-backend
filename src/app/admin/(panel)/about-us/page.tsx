...
export default function AdminAboutUsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<AboutUsSection[]>([])
  ...
  const pendingDeleteRef = useRef<{ id: string; item: AboutUsSection; timer: number } | null>(null)
  ...
  function deleteWithUndo(id: string) {
    ...
    // önce eski pending varsa finalize et (çakışmasın)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Hakkımızda Bölümleri</h1>
          <p className="text-slate-500 mt-1">Hakkımızda sayfasında görünecek bölümleri buradan yönet</p>
        </div>
                  ...
                  <tr key={it.id} className="hover:bg-slate-50/60 transition">
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="inline-flex items-center gap-2">
                        ...
                        <button
                          onClick={() => router.push(`/admin/about-us/${encodeURIComponent(it.id)}/edit`)}
              ...
              {!loading && sorted.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-5 py-10 text-center">
                    ...
                    <div className="mt-1 text-sm text-slate-500">“Yeni Bölüm” ile hakkımızda içeriğini ekleyebilirsin.</div>
                    <div className="mt-4">
                      <Link
                        href="/admin/about-us/create"
                        className="inline-flex items-center gap-2 rounded-2xl bg-slate-900 text-white px-4 py-2.5 text-sm font-semibold shadow-sm hover:bg-slate-800 transition"
