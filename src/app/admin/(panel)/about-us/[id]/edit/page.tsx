...
/* ====================== PAGE ====================== */
export default function AdminAboutUsEditPage() {
  ...
  // change tracking (disable save if no changes)
  const initialRef = useRef<{ title: string; content: AboutContent } | null>(null)
  ...
  // ✅ REAL LOAD: GET /api/admin/about-us/:id
  useEffect(() => {
    let mounted = true
    async function run() {
      try {
        setLoading(true)
        const data = (await apiGet(`/api/admin/about-us/${encodeURIComponent(id)}`)) as AboutApiItem

        if (!mounted) return
        const nextTitle = (data?.title || "").toString()
        const nextContent = (data?.content as AboutContent) || defaultContent()

        setTitle(nextTitle)
        setContent(nextContent)

        initialRef.current = { title: nextTitle, content: nextContent }
      } catch (e: any) {
        if (!mounted) return
  ...
  const hasChanges = useMemo(() => {
    ...
    try {
      return JSON.stringify(init.content) !== JSON.stringify(content)
    } catch {
      return true
    }
  }, [title, content])
  ...
  async function onSave() {
    ...
    if (!hasChanges) {
      showToast("success", "Değişiklik yok", "Kaydedilecek yeni bir şey bulunamadı.")
      return
    }
    ...
    try {
      ...
      const updated = await apiPatch(`/api/admin/about-us/${encodeURIComponent(id)}`, payload)

      // server geri ne döndürürse döndürsün, local state'yi “saved” say
      initialRef.current = { title: title.trim(), content }

      showToast("success", "Kaydedildi", "Güncelleme başarılı.")
      // istersen updatedAt vb. gösterebilirsin: console.log(updated)
      void updated
    } catch (e: any) {
  ...
  function onResetDefaults() {
    ...
    showToast("success", "Default içerik yüklendi", "Kaydet’e basarsan bu içerik kayıt olur.")
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Hakkımızda • Düzenle</h1>
          <p className="text-slate-500 mt-1">Mevcut kaydı güncelle</p>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={!hasChanges ? "Değişiklik yok" : ""}
          >
      ...
      <div className="rounded-2xl border border-slate-200/70 bg-white/80 p-5">
        <div className="flex items-center justify-between gap-3">
          <div className="text-xs font-semibold text-slate-600">Kayıt Başlığı</div>
          <div className="text-xs text-slate-500">{hasChanges ? "• Değişiklik var" : "• Kaydedildi"}</div>
        </div>

        <input
          value={title}
          onChange={(e) => setTitle(e.target.value)}
          className="mt-2 w-full rounded-2xl border border-slate-200/80 bg-white/80 px-3 py-2.5 text-sm font-semibold outline-none focus:ring-2 focus:ring-orange-200"
      ...
      {/* Loading */}
      {loading ? (
        <div className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 text-sm text-slate-600">Yükleniyor...</div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-6 space-y-6">
            <Section title="Üst Bant">
              <TwoCol>
                <Field
                  label="Başlık"
                  value={content.topBandTitle}
                  onChange={(v) => setContent((p) => ({ ...p, topBandTitle: v }))}
                />
                <Field
                  label="Alt yazı"
            ...
            <Section title="Değerler Bölümü">
              ...
              <Field label="Başlık" value={content.valuesTitle} onChange={(v) => setContent((p) => ({ ...p, valuesTitle: v }))} />
              <TextArea label="Alt açıklama" value={content.valuesDesc} onChange={(v) => setContent((p) => ({ ...p, valuesDesc: v }))} />
            ...
            <Section title="İletişim CTA">
              <Field label="Başlık" value={content.contactTitle} onChange={(v) => setContent((p) => ({ ...p, contactTitle: v }))} />
              <Field label="Açıklama" value={content.contactDesc} onChange={(v) => setContent((p) => ({ ...p, contactDesc: v }))} />
              <TwoCol>
                <Field
          ...
          <div className="lg:col-span-6">
            ...
            <div className="mt-4 rounded-2xl border border-slate-200/70 bg-white/70 p-4 text-sm text-slate-600">
              ...
              <div className="mt-1">
                Bu sayfa artık gerçek API kullanıyor:
                <span className="font-semibold text-slate-900"> GET/PATCH /api/admin/about-us/:id</span>
              </div>
            </div>
          </div>
        </div>
