...
import { Plus, Trash2, Pencil, CheckCircle2, RefreshCcw, Undo2, Shield } from "lucide-react"
...
export default function AdminRulesListPage() {
  ...
  const [items, setItems] = useState<RulesDoc[]>([])
  ...
  async function refresh(silent = false) {
    try {
      setLoading(true)
      const data = await apiGet("/api/admin/rules")
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const pendingDeleteRef = useRef<{ id: string; item: RulesDoc; timer: number } | null>(null)

  async function activate(id: string) {
    try {
      await apiPatch(`/api/admin/rules/${encodeURIComponent(id)}/activate`)
      setItems((prev) => prev.map((x) => ({ ...x, isActive: x.id === id })))
      showToast({open: true, kind: "success", title: "Aktif kurallar güncellendi", desc: "Web tarafında artık bu kayıt görünecek." })
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } catch (e: any) {
  ...
  function deleteWithUndo(id: string) {
    ...
    const timer = window.setTimeout(async () => {
      try {
        pendingDeleteRef.current = null
        await apiDelete(`/api/admin/rules/${encodeURIComponent(id)}`)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Kurallar (Rules)</h1>
          <p className="text-slate-500 mt-1">Web /kurallar sayfasında görünecek kuralları buradan yönet</p>
        </div>
                  ...
                  <tr key={it.id} className="hover:bg-slate-50/60 transition">
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="inline-flex items-center gap-2">
                        ...
                        <button
                          onClick={() => router.push(`/admin/rules/${encodeURIComponent(it.id)}/edit`)}
              ...
              {!loading && sorted.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-5 py-10 text-center">
                    ...
                    <div className="mt-1 text-sm text-slate-500">“Yeni Kural Seti” ile ilk kuralları ekleyebilirsin.</div>
                    <div className="mt-4">
                      <Link
                        href="/admin/rules/create"
                        className="inline-flex items-center gap-2 rounded-2xl bg-slate-900 text-white px-4 py-2.5 text-sm font-semibold shadow-sm hover:bg-slate-800 transition"
