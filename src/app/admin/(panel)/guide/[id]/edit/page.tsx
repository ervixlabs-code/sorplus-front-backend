...
export default function AdminGuideEditPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        setLoading(true)
        const data = (await apiGet(`/api/admin/guides/${encodeURIComponent(id)}`)) as GuideApiItem
        if (!mounted) return
  ...
  async function onSave() {
    try {
      ...
      setSaving(true)
      await apiPatch(`/api/admin/guides/${encodeURIComponent(id)}`, {
        title: title.trim(),
        content,
      })
  ...
  function onResetDefaults() {
    ...
    setContent(d)
    showToast("success", "Default içerik yüklendi", "Kaydet'e basarsan DB'ye yazılır.")
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Mevcut rehber kaydını güncelle</p>
...
>
  ...
  Webde Gör
      ...
      {loading ? (
        ...
        /* Editor (create tasarımı birebir) */
          ...
          <div className="p-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="rounded-2xl border border-slate-200/70 bg-white/70 p-4 text-sm text-slate-600">
                ...
                <div className="mt-1">Edit sayfası create ile birebir. Kaydet ile PATCH atıyoruz.</div>
              </div>
            </div>
          </div>
        </div>
